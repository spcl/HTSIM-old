...
template <class PullPkt> class BasePullQueue {
  public:
    BasePullQueue();
  ...
  protected:
    int64_t _preferred_flow; // flow_id is uint32_t, int64_t can store this,
                             // plus -1
};

template <class PullPkt> class FifoPullQueue : public BasePullQueue<PullPkt> {
  public:
    FifoPullQueue();

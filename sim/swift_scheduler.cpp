...
void BaseScheduler::receivePacket(Packet &pkt) {
    ...
    // cout << "recv_packet " << this << " flow_id " << flow_id << " count " <<
    // _pkt_count << " flow_count " << _queue_counts[flow_id] << endl;
    if (pkt.type() == SWIFT) {
...
void BaseScheduler::doNextEvent() { completeService(); }
...
Packet *FifoScheduler::next_packet() { return _queue.back(); }

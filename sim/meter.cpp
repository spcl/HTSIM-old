...
void Meter::doNextEvent() { completeService(); }
...
meter_output Meter::execute(Packet &pkt) {
    ...
    // cout << "Adding credit " << delta/service_rate << "total " << _credit <<
    // "delta " << delta << " service rate " << service_rate << " Times " <<
    // eventlist().now() << " " << _endpesitrim << " " << _endsemipesitrim <<
    // endl;
...
mem_b Meter::queuesize() { return _queuesize; }

simtime_picosec Meter::serviceTime() { return _queuesize * _ps_per_byte; }

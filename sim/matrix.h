...
template <typename T> void deletep(T &) {}

template <typename T> void deletep(T *&ptr) {
    delete ptr;
    ptr = 0;
}
...
template <typename T> inline T &Matrix2d<T>::operator()(size_t x, size_t y) {
    // do bounds checking
    assert(x < _xsize && y < _ysize);
    return _data[(x * _ysize) + y];
}

template <typename T> inline T &Matrix2d<T>::at(size_t x, size_t y) {
    // do bounds checking
    assert(x < _xsize && y < _ysize);
    return _data[(x * _ysize) + y];
}
...
template <typename T> inline T &Matrix3d<T>::at(size_t x, size_t y, size_t z) {
    // do bounds checking
    assert(x < _xsize && y < _ysize && z < _zsize);
    return _data[((x * _ysize) + y) * _zsize + z];
}

...
void TcpSrc::connect(const Route &routeout, const Route &routeback,
    ...
    assert(_route);
    _sink = &sink;
    _flow.set_id(get_id()); // identify the packet flow with the TCP source that
                            // generated it
...
void TcpSrc::receivePacket(Packet &pkt) {
    ...
    // assert(seqno >= _last_acked);  // no dups or reordering allowed in this
    // simple simulator
    ...
    //  cout << "Base "<<timeAsMs(_base_rtt)<< " RTT " << timeAsMs(_rtt)<< "
    //  Queued " << queued_packets << endl;
    ...
    if (seqno > _last_acked) { // a brand new ack
        ...
        //   cout << "ACK "<<cnt<<" packets on " << _flow.id << " " <<
        //   _highest_sent+1 << " packets in flight " << _sent_packets.crt_count
        //   << " diff " << (_highest_sent+_mss-_last_acked)/1000 << "
        //   last_acked " << _last_acked << " at " <<
        //   timeAsMs(eventlist().now()) << endl;
#endif

        if (!_in_fast_recovery) { // best behaviour: proper ack of a new packet,
...
void TcpSrc::send_packets() {
    ...
    while ((_last_acked + c >= _highest_sent + _mss) &&
        ...
        if (data_seq > _highest_data_seq)
            ...
            //      cout << "Transmit packet on " << _flow.id << " " <<
            //      _highest_sent+1 << "[" << data_seq << "] packets in flight "
            //      << _sent_packets.crt_count << " diff " <<
            //      (_highest_sent+_mss-_last_acked)/1000 << " last_acked " <<
            //      _last_acked << " at " << timeAsMs(eventlist().now()) <<
            //      endl;
#endif
...
void TcpSrc::retransmit_packet() {
    ...
    //  else
    //  cout << "Retransmit packet on " << _flow.id << " " << _last_acked+1 << "
    //  " << data_seq << endl;
#endif

...
void TcpSrc::rtx_timer_hook(simtime_picosec now) {
    if (_highest_sent == 0)
        return;
    if (now <= _last_sent_time + timeFromMs(1000))
        return;
    // RTX timer expired
    if (_logger)
...
TcpSink::TcpSink() : Logged("sink") {}

void TcpSink::connect(TcpSrc &src, route_t &route) {
    _src = &src;
    _route = &route;
    _cumulative_ack = 0;
}

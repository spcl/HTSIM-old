...
void STrackSrc::adjust_cwnd(simtime_picosec delay, STrackAck::seq_t ackno) {
    ...
    // STrack cwnd calculation.  Doing this here does it for every ack, no
    // matter if we're in fast recovery or not.  Need to be careful.
...
void STrackSrc::applySTrackLimits() {
    ...
    if (_strack_cwnd < mss()) {
        ...
        // cout << "strack_cwnd " << ((double)_sub->_strack_cwnd)/_mss << "
        // pacing " << timeAsUs(_pacing_delay) << "us" << endl;
    } else {
        _pacing_delay = 0;
    }
}
...
void STrackSrc::handle_ack(STrackAck::seq_t ackno) {
    ...
    if (ackno > _last_acked) {             // a brand new ack
        ...
        if (!_in_fast_recovery) {
            ...
            // expecting it. clear timers.  strack has already calculated new
            // cwnd.
            _last_acked = ackno;
            _dupacks = 0;
        ...
        if (ackno >= _recoverq) {
            ...
            //  in NewReno, we'd reset the cwnd here, but I think in strack we
...
int STrackSrc::send_packets() {
    ...
    int sent_count = 0;
    // cout << eventlist().now() << " " << nodename() << " cwnd " <<
    // _strack_cwnd << " + " << _inflate << endl;
    if (!_established) {
    ...
    while ((_last_acked + c >= _highest_sent + mss()) && more_data_available()) {
        ...
        if (_RFC2988_RTO_timeout == timeInf) { // RFC2988 5.1
            _RFC2988_RTO_timeout = eventlist().now() + _rto;
            // cout << timeAsUs(eventlist().now()) << " " << nodename() << " RTO
            // at " << timeAsUs(_RFC2988_RTO_timeout) << "us" << endl;
        }
    }
    return sent_count;
}
...
void STrackSrc::receivePacket(Packet &pkt) {
    ...
    if (now - _last_BDP_update > _base_rtt) {
        ...
        if (now - _last_active < _base_rtt) {
            ...
            // we didn't get any acks in the last RTT, so achieved BDP is not
            // valid.
            _achieved_BDP = 0;
            _rx_count = acked_bytes;
        }
...
void STrackSrc::connect(const Route &routeout, const Route &routeback, STrackSink &sink, simtime_picosec starttime) {
    ...
    // (immediately) be used sub->connect(sink, routeout, routeback, get_id(),
    // _scheduler);
    sink.connect(*this, routeback);
    Route *cloned_rt = routeout.clone(); // make a copy, as we may be switching
                                         // routes and don't want to append the
                                         // sink more than once
...
simtime_picosec STrackSrc::targetDelay(const Route &route) {
    ...
    _sub->_strack_cwnd is in bytes
    //cout << "fs_delay " << fs_delay << " range " << _fs_range << "
    _sub->_strack_cwnd/_mss " << _sub->_strack_cwnd/_mss << " sqrt " <<
    sqrt(_sub->_strack_cwnd/_mss) << " beta " << _fs_beta << endl;
...
void STrackPacer::just_sent() { _last_send = eventlist().now(); }
...
uint64_t STrackSink::cumulative_ack() {
    // this is needed by some loggers.  If we ever need it, figure out what it
    // should really return
    return _src ? _cumulative_ack + _src->_mss : 0;
}

uint32_t STrackSink::drops() { return _src ? _src->drops() : 0; }

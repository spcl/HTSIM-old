...
DragonFlyTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse) {
    ...
    if (HOST_TOR(src) == HOST_TOR(dest)) {
        ...
        // dstswitch from dstgroup which has an incoming path from srcgroup.
        ...
        if (HOST_TOR(src) != srcswitch) {
            /*When SRC HOST TOR does not have a direct path to destination
        ...
        if (dstswitch != HOST_TOR(dest)) {
             ...
             * path to the appropriate TOR switch*/
        ...
        /*routeback->push_back(queues_host_switch[dest][HOST_TOR(dest)]);
          ...
          if (HOST_TOR(src)!=srcswitch){
          //When SRC HOST TOR does not have a direct path to destination group,
          take local path to the appropriate switch
        ...
        */
        ...
        for (uint32_t p = 0; p < _no_of_groups; p++) {
            ...
            // cout << "Groups " << srcgroup  << "  "  << intergroup << " " <<
            // dstgroup << endl;

            uint32_t srcswitch, dstswitch, interswitch1, interswitch2;
            // find srcswitch from srcgroup which has a path to intergroup and
            // dstswitch from intergroup which has an incoming path from
            // srcgroup.
            ...
            if (interswitch1 != interswitch2) {
                /*When interswitch1 does not have a direct path to destination
                 * group, take local path to the appropriate switch*/

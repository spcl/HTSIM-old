...
uint32_t RTT = 1; // this is per link delay in us; identical RTT microseconds =
                  // 0.001 ms
...
int main(int argc, char **argv) {
...
#ifdef FAT_TREE
    /*
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, linkspeed,
    ...
    */
    ...
    for (uint32_t c = 0; c < all_conns->size(); c++) {
        ...
        // ReorderBufferLoggerSampling(timeFromMs(0.01), eventlist);
    ...
    for (src_i = swift_srcs.begin(); src_i != swift_srcs.end(); src_i++) {
        cout << "Src, sent: " << (*src_i)->_packets_sent << "[new: " <<
    (*src_i)->_new_packets_sent << " rtx: " << (*src_i)->_rtx_packets_sent << "]
    nacks: " << (*src_i)->_nacks_received << " pulls: " <<
    (*src_i)->_pulls_received << " paths: " << (*src_i)->_paths.size() << endl;
    }
    for (src_i = swift_srcs.begin(); src_i != swift_srcs.end(); src_i++) {
        (*src_i)->print_stats();
        }*/

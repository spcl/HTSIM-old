...
class SwiftSubflowSrc : public EventSource {
  ...
  protected:
    ...
    uint32_t _inflate; // how much we're currently extending cwnd based off dup
                       // ack arrivals.
    uint64_t _recoverq;
    ...
    uint64_t _last_acked; // ack number of the last packet we received a
                          // cumulative ack for
    uint16_t _dupacks;
...
class SwiftSrc : public PacketSink, public EventSource {
  ...
  public:
    ...
    uint64_t _highest_dsn_sent; // seqno is in bytes - data sequence number, for
                                // MPSwift
    uint64_t _flow_size;
...
class SwiftSink : public PacketSink, public DataReceiver, public Logged {
  ...
  public:
    ...
    list<SwiftAck::seq_t> _received;       /* list of packets above a hole, that
                                            we've received */
    set<SwiftPacket::seq_t> _dsn_received; // use a map because multipath
                                           // packets will arrive out of order

    uint64_t cumulative_ack() {
        // this is needed by some loggers.  If we ever need it, figure out what

...
void HPCCSrc::connect(Route *routeout, Route *routeback, HPCCSink &sink,
    ...
    _sink = &sink;
    _flow.set_id(get_id()); // identify the packet flow with the HPCC source
                            // that generated it
    _flow._name = _name;
...
void HPCCSrc::processNack(const HPCCNack &nack) {
    ...
    if (_flow_size && _highest_sent > _flow_size && _last_acked < _flow_size) {
        ...
        // flow size but now a packet in the last window was lost.
        eventlist().sourceIsPendingRel(*this, 0);
    }
...
void HPCCSrc::processAck(const HPCCAck &ack) {
    ...
    */
    ...
    // cout << "INT Entries " << ack._int_hop << " qs " <<
    // ack._int_info[0]._queuesize << " TS " << timeAsUs(ack._int_info[0]._ts)
    // << " TX " << ack._int_info[0]._txbytes << endl;
...
void HPCCSrc::processPause(const EthPausePacket &p) {
    if (p.sleepTime() > 0) {
        ...
        // cout << "Source " << str() << " PAUSE " <<
        // timeAsUs(eventlist().now()) << endl; assert(_state_send != PAUSED);
        _state_send = PAUSED;
    } else {
        // we are allowed to send!
        // assert(_state_send != READY);
        _state_send = READY;
        // cout << "Source " << str() << " RESUME " <<
...
void HPCCSrc::send_packet() {
    ...
    if (_flow_size && _highest_sent + _mss >= _flow_size) {
        last_packet = true;
        // cout << _name << " sending last packet with SEQNO " <<
...
void HPCCSrc::doNextEvent() {
    ...
    if (_flow_size && _highest_sent >= _flow_size) {
        // cout << _name << " stopping send coz highest_sent is " <<
        // _highest_sent << endl;
        return;
    }
...
void HPCCSink::receivePacket(Packet &pkt) {
    ...
    if (seqno > _cumulative_ack + 1) {
        ...
        // cout << "Wrong seqno received at HPCC SINK " << seqno << " expecting
        // " << _cumulative_ack << endl;
        return;
    }

    int size = p->size() - hpcc_acksize;

...
int start_lgs(std::string filename_goal) {
    ...
    while (!aq.empty() || new_events) {
        ...
        case OP_LOCOP: {
            if (print)
                printf("[%i] found loclop of length %lu - t: %lu (CPU: %i)\n", elem.host, (ulint)elem.size,
                       (ulint)elem.time, elem.proc);
        ...
        case OP_SEND: { // a send op
            ...
            if (std::max(nexto[elem.host][elem.proc], nextgs[elem.host][elem.nic]) <=
                ...
                nextgs[elem.host][elem.nic] =
                        elem.time + g + (elem.size - 1) * G; // TODO: G should be charged in network layer only
                // tlviz.add_osend(elem.host, elem.time, elem.time+o+ (elem.size-1)*O, elem.proc);
                // tlviz.add_noise(elem.host, elem.time+o+ (elem.size-1)*O, elem.time + o + (elem.size-1)*O+ noise,
                // elem.proc);
                ...
                elem.time = elem.time + o + L; // arrival time of first byte only (G will be charged at receiver ...
                                               // +(elem.size-1)*G; // arrival time
#ifdef STRICT_ORDER
                ...
                if (elem.size <= S) { // eager message
                    if (print)
                        printf("-- [%i] eager -- satisfy local requires at t: %lu\n", h,
                               (long unsigned int)elem.starttime + o);
            ...
            if (match_attempts >= 0) { // found it in local UQ
                ...
                if (elem.size > S) { // rendezvous - free remote request
                    ...
                    parser.schedules[matched_elem.src].MarkNodeAsDone(
                            matched_elem.offset); // this must be the offset of the remote packet!
                    ...
                    // tlviz.add_transmission(elem.host, matched_elem.src, matched_elem.starttime+o, elem.time,
                    // elem.size, G);
                }
        ...
        case OP_MSG: {
            ...
            // NUMBER of elements that were searched during message matching
            int32_t match_attempts;
            if ((earliestfinish = net.query(elem.starttime, elem.time, elem.target, elem.host, elem.size,
                                            &elem.handle)) <= elem.time /* msg made it through network */
                && std::max(nexto[elem.host][elem.proc], nextgr[elem.host][elem.nic]) <=
                           elem.time /* local o,g available! */) {
                if (print)
                ...
                nexto[elem.host][elem.proc] =
                        elem.time + o + noise +
                        std::max((elem.size - 1) * O,
                                 (elem.size - 1) * G) /* message is only received after G is charged !! TODO: consuming
                                                         o seems a bit odd in the LogGP model but well in practice */
                        ;
                ...
                if (match_attempts >= 0) { // found it in RQ
                    ...
                    // tlviz.add_orecv(elem.host, elem.time+(elem.size-1)*G-(elem.size-1)*O,
                    // elem.time+o+std::max((elem.size-1)*O,(elem.size-1)*G), elem.proc); tlviz.add_noise(elem.host,
                    // elem.time+o+std::max((elem.size-1)*O,(elem.size-1)*G),
                    // elem.time+o+noise+std::max((elem.size-1)*O,(elem.size-1)*G), elem.proc);
                    //  satisfy local requires
                ...
                elem.time =
                        std::max(std::max(nexto[elem.host][elem.proc], nextgr[elem.host][elem.nic]), earliestfinish);
                if (print)
                ...
                case OP_SEND:
                    ...
                    if (print)
                        printf("%i (%i,%i) send to: %i, tag: %i, size: %lu, time: %lu, offset: %i\n", host,
                               freeop->proc, freeop->nic, freeop->target, freeop->tag, (long unsigned int)freeop->size,
    ...
    printf("PERFORMANCE: Processes: %i \t Events: %lu \t Time: %lu s \t Speed: %.2f ev/s\n", p,
           (long unsigned int)aqtime, (long unsigned int)diff, (float)aqtime / (float)diff);

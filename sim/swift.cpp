...
void SwiftSubflowSrc::adjust_cwnd(simtime_picosec delay,
    ...
    /*
      if (_src.plb()) {
      simtime_picosec td = _src.targetDelay(0, *_route); // target delay
      ignoring cwnd component
      //cout << nodename() << " delay " << delay << "rep td " << td << endl;
      if (delay > td) {
...
void SwiftSubflowSrc::handle_ack(SwiftAck::seq_t ackno) {
    ...
    if (ackno > _last_acked) {             // a brand new ack
        ...
        if (ackno >= _highest_sent) {
            ...
            _highest_sent = ackno;
            // cout << timeAsUs(now) << " " << nodename() << " highest_sent now
            // " << _highest_sent << endl;
...
bool SwiftSubflowSrc::send_next_packet() {
    ...
    if (_src.more_data_available()) {
        ...
        // cout << timeAsUs(eventlist().now()) << " " << nodename() << " no more
        // data" << endl;
        return false;
    }
...
void SwiftSubflowSrc::receivePacket(Packet &pkt) {
    ...
    // << ackno << " ts_echo " << timeAsUs(ts_echo) << endl; cout <<
    // timeAsUs(eventlist().now()) << " " << nodename() << " highest_sent was  "
    // << _sub->_highest_sent << endl;
...
SwiftSrc::SwiftSrc(SwiftRtxTimerScanner &rtx_scanner, SwiftLogger *logger,
    ...
    // cout << _fs_range / ((1.0 / sqrt(_fs_min_cwnd)) -  (1.0 /
    // sqrt(_fs_max_cwnd))) << endl;
...
void SwiftSrc::set_cwnd(uint32_t cwnd) {
    for (size_t i = 0; i < _subs.size(); i++) {
        ...
        // cout << eventlist().now() << " " << _subs[i]->nodename() << "
        // set_cwnd, cwnd " << _subs[i]->_swift_cwnd << endl;
    }
...
void SwiftSrc::set_paths(vector<const Route *> *rt_list) {
    ...
    for (size_t i = 0; i < no_of_paths; i++) {
        ...
        if (!_scheduler) {
            ...
            // sanity check all paths share the same scheduler.  If we ever want
            // to use multiple NICs, this will need fixing
...
void SwiftSrc::startflow() {
    for (size_t i = 0; i < _subs.size(); i++) {
        ...
        // cout << eventlist().now() << " " << _subs[i]->nodename() << " (sub"
        // << i << ") started, cwnd " << _subs[i]->_swift_cwnd << endl;
        _subs[i]->send_packets();
    }
}
...
void SwiftSrc::multipath_connect(SwiftSink &sink, simtime_picosec starttime,
    ...
    for (uint32_t i = 0; i < no_of_subflows; i++) {
        ...
        if (_paths.size() < no_of_subflows) {
            cout << "More subflows requested than paths available - there will "
                    "be duplicates\n";
        }
...
const string &SwiftSubflowSink::nodename() { return _sink.nodename(); }
...
uint32_t SwiftSink::drops() { return _src ? _src->drops() : 0; }

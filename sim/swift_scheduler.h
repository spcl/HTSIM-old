...
class BaseScheduler : public BaseQueue {
  public:
    BaseScheduler(linkspeed_bps bitrate, EventList &eventlist,
                  QueueLogger *logger);
  ...
  protected:
    ...
    map<flowid_t, ScheduledSrc *>
            _srcs; // map from flow id to SubflowSrc for callbacks
};

class FifoScheduler : public BaseScheduler {
  public:
    FifoScheduler(linkspeed_bps bitrate, EventList &eventlist,
                  QueueLogger *logger);
...
class FairScheduler : public BaseScheduler {
  public:
    FairScheduler(linkspeed_bps bitrate, EventList &eventlist,
                  QueueLogger *logger);
    virtual void enqueue(Packet &pkt);
    virtual Packet *next_packet();
    virtual Packet *dequeue();

...
uint32_t SwiftSrc::effective_window() { return _in_fast_recovery ? _ssthresh : _swift_cwnd; }
...
void SwiftSrc::send_packets() {
    ...
    while ((_last_acked + c >= _highest_sent + _mss) && (_highest_sent + _mss <= _flow_size + 1)) {
        ...
        if (_RFC2988_RTO_timeout == timeInf) { // RFC2988 5.1
            _RFC2988_RTO_timeout = eventlist().now() + _rto;
            cout << timeAsUs(eventlist().now()) << " " << nodename() << " RTO at " << timeAsUs(_RFC2988_RTO_timeout)
                 << "us" << endl;
        }
    }
}
...
void SwiftSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << nodename() << " At " << now / (double)1000000000 << " RTO "
         ...
         << _last_acked / _mss << " HSENT " << _highest_sent << " CWND " << _swift_cwnd / _mss << " FAST RECOVERY? "
...
SwiftSink::SwiftSink() : DataReceiver("sink"), _cumulative_ack(0), _packets(0) { _nodename = "tcpsink"; }
...
void SwiftRtxTimerScanner::registerSwift(SwiftSrc &tcpsrc) { _tcps.push_back(&tcpsrc); }

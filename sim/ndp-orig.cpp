...
void NdpSrc::processAck(const NdpAck &ack) {
    ...
    */
    ...
    if (cum_ackno >= _flow_size) {
        cout << "Flow " << _name << " finished at " << timeAsUs(eventlist().now()) << " total bytes " << cum_ackno
             << endl;
        return;
    }
...
void NdpSrc::send_packet(NdpPull::seq_t pacer_no) {
    ...
    if (!_rtx_queue.empty()) {
        ...
        case SCATTER_ECMP: {
            /*
            if (_log_me) {
                cout << "Highest_sent: " << _highest_sent << " Flow_size: " <<
            _flow_size << endl;
            }
            */
            assert(_paths.size() > 0);
        ...
        case SINGLE_PATH:
            p = NdpPacket::newpkt(_flow, *_route, _highest_sent + 1, pacer_no, _mss, false, 1, last_packet);
            break;
        case NOT_SET:
            abort();
        }
...
void NdpSrc::retransmit_packet() {
    ...
    for (j = rtx_list.begin(); j != rtx_list.end(); j++) {
        ...
        case SINGLE_PATH:
            p = NdpPacket::newpkt(_flow, *_route, seqno, 0, _mss, true, _paths.size(), last_packet);
            break;
        case NOT_SET:
            abort();
        }
    ...
    if (_pull_queue.empty()) {
        ...
        if (delta >= drain_time) {
            ...
            if (_log_me) {
                ...
                _excess_count++;
                /*		cout << "Mean excess: " << _total_excess /
                _excess_count << endl; if (ack->type() == NDPACK) { cout << "Ack
                " <<  (((NdpAck*)ack)->ackno()-1)/9000 << " excess " << excess
                << " (no queue)\n"; } else if (ack->type() == NDPNACK) { cout <<
                "Nack " << (((NdpNack*)ack)->ackno()-1)/9000 << " excess " <<
                excess << " (no queue)\n"; } else { cout << "WTF\n";
                    }*/
            }
...
void NdpPullPacer::release_pulls(uint32_t flow_id) { _pull_queue.flush_flow(flow_id); }
...
void NdpPullPacer::enqueue_pull(NdpPull *pkt) {
    ...
    if (_log_me) {
        cout << "Enqueue pull, pull queue size is " << _pull_queue.pull_count() << " should_tx is " << should_tx
             << " delta " << timeAsUs(delta) << " drain " << timeAsUs(_packet_drain_time) << endl;
    }

...
void MultipathTcpSrc::receivePacket(Packet &pkt) {
    ...
    //  cout << "Data SEQ: " << seqno << " at " << timeAsMs(eventlist().now())
    //  <<endl;

    if (seqno <= _last_acked)
        return;

    // there is a chance that we can send packets on all subflows - we should
    // try them out.
    _last_acked = seqno;
...
int MultipathTcpSrc::getDataSeq(uint64_t *seq, TcpSrc *subflow) {
    // if cwnd is small (high chance of RTO) and there is unacked data SEQ, give
    // it that. if there is a possibility of stall (know rwnd) then give it
    // redundant data.
...
#endif
        // we should retransmit the oldest seqno which hasn't been sent on this
        // subflow
...
uint32_t MultipathTcpSrc::inflate_window(uint32_t cwnd, int newly_acked,
    ...
    case COUPLED_INC:
        ...
        //    cout << timeAsMs(eventlist().now()) << " ID " << id << " Tmp "<<
        //    tmp << " tmp2 " << tmp2 << " tt " << tmp_float << " tcp inc " <<
        //    tcp_inc << " alfa " << a << endl;
...
double MultipathTcpSrc::compute_a() {
    ...
    if (flow0->_rtt_avg > timeFromMs(0) && flow1->_rtt_avg > timeFromMs(1))
        ...
        // cout << "R1 " << flow0->_rtt_avg/1000000000 << " R2 " <<
        // flow1->_rtt_avg/1000000000 << endl;
#else
    rdelta = (double)flow0->_rtt / flow1->_rtt;

    // cout << "R1 " << flow0->_rtt/1000000000 << " R2 " <<
    // flow1->_rtt/1000000000 << endl;
#endif
...
void MultipathTcpSink::doNextEvent() {
    ...
    // cout << "Receive buffer should be " << new_window << " crt " <<
    // m->_receive_window << endl;
    // }
...
void MultipathTcpSink::receivePacket(Packet &pkt) {
    ...
    // cout << "at " << timeAsMs(eventlist().now()) << " Received on " <<
    // p->flow().id << " " << p->seqno() << " " << p->data_seqno() << " waiting
    // for " << _cumulative_ack+1 << endl;

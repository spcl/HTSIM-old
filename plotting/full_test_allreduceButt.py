...
def getRunString(algo, exp, os_ratio):
    ...
    if (algo == "SMaRTT"):
        ...
        string_to_run="../sim/datacenter/htsim_ndp -o uec_entry -nodes {} -k {} -kmin 20 -kmax 80 -ratio_os_stage_1 1 -cwnd 444600 -q 444600 -strat ecmp_host_random2_ecn -linkspeed {} -mtu {} -seed 95 -hop_latency 700 -switch_latency 0 -tm ../sim/datacenter/connection_matrices/alltoall_normal_tm_16/{} -collect_data {} -number_entropies 256 > {}/{}/P".format(num_nodes, os_ratio, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)
...
def getRunStringUnbalanced(algo, exp, os_ratio):
    ...
    if (algo == "SMaRTT"):
        ...
        string_to_run="../sim/datacenter/htsim_ndp -o uec_entry -nodes {} -k {} -kmin 20 -kmax 80 -ratio_os_stage_1 1 -cwnd 448500 -q 448500 -strat ecmp_host_random2_ecn -linkspeed {} -mtu {} -seed 95 -hop_latency 700 -switch_latency 0 -tm ../sim/datacenter/connection_matrices/alltoall_normal_tm_16/{} -collect_data {} -number_entropies 256 > {}/{}/P".format(num_nodes, os_ratio, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)

...
for i, data in enumerate(all_data):
    ax = sns.lineplot(x=data, y=cumulative_probabilities[i], label=labels[i], linewidth = 3.5)

ax.set_xticklabels([str(i) for i in ax.get_xticks()], fontsize = 17)
ax.set_yticklabels([str(round(i,1)) for i in ax.get_yticks()], fontsize = 17)

# Add a vertical dashed line at x=1000 with lower opacity
plt.axvline(x=best_time, linestyle='--', color='gray', alpha=0.5, linewidth = 3.5)

# Add text next to the vertical line
plt.text(best_time - 1.3, 0.4, 'Ideal Completion Time', rotation=90, va='center', color='gray',fontsize=17)
...
plt.xlabel('Flow Completion Time (us)',fontsize=19)
plt.ylabel('CDF (%)',fontsize=19)
plt.title('Permutation 4:1 oversubscription 1MiB\nLink Speed 800Gbps - 4KiB MTU',fontsize=19)

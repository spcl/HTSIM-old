...
# Test Name
FOLDER="FOLDER_400_FULL_TEST_PERMUTATION_OS2"
...
def getRunString(algo, exp):
    ...
    if (algo == "SMaRTT"):
            string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k 4 -ratio_os_stage_1 2 -algorithm delayB -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal {} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "SMaRTT_ECN"):
        string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k 4 -ratio_os_stage_1 2 -algorithm ecn -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal {} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "SMaRTT_RTT"):
        string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k 4 -ratio_os_stage_1 2 -algorithm rtt -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal {} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "Swift"):
        string_to_run="../sim/datacenter/htsim_swift_entry -k 4 -ratio_os_stage_1 2 -o uec_entry -nodes {} -q 448500 -strat ecmp_host_random2_ecn -kmin 20 -target_rtt_percentage_over_base 50 -kmax 80 -linkspeed {} -mtu {} -seed 95 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 0 -goal {} -number_entropies 256  -collect_data {}  > {}/{}/P".format(num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "EQDS"):
        string_to_run="../sim/datacenter/htsim_ndp_entry_modern -o uec_entry -nodes {} -k 4 -kmin 20 -kmax 80 -ratio_os_stage_1 2 -cwnd 448500 -q 448500 -strat ecmp_host_random2_ecn -linkspeed {} -mtu {} -seed 95 -hop_latency 700 -switch_latency 0 -goal {} -collect_data {} -number_entropies 256 > {}/{}/P".format(num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)

...
RES_FOLDER="Parameter_Analysis_Permutation_NoOS_2000_400G"
CMD="rm -r ${RES_FOLDER}"
echo ${CMD}
eval ${CMD}
...
# Threee Incast Sizes
for Incast_Message_Size in permutation_128_2000.bin
do
    for KMin in 20 50
    do
...
CMD="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -nodes 128 -q 432500 -strat perm -kmin 20 -kmax 80 -linkspeed 400000 -mtu 2048 -seed 99 -queue_type composite_bts -hop_latency 700 -switch_latency 0 -reuse_entropy 0 -goal ${Incast_Message_Size} -ignore_ecn_data 1 -ignore_ecn_ack 1 -number_entropies -1 > ${RES_FOLDER}/${FILE_NAME}"
echo ${CMD}
eval ${CMD}
CMD="python3 generate_report.py --input_file=${FILE_NAME} --folder=${RES_FOLDER} --parameter_analysis=1 --complex_name=${FILE_NAME}"
echo ${CMD}
...
FILE_NAME="ndp.tmp"
CMD="../sim/datacenter/htsim_ndp_entry_modern -o uec_entry -nodes 128 -cwnd 432500 -q 432500 -strat perm -linkspeed 400000 -mtu 2048 -seed 99 -hop_latency 700 -switch_latency 0 -goal ${Incast_Message_Size} > ${RES_FOLDER}/${FILE_NAME}"
echo ${CMD}
eval ${CMD}
CMD="python3 generate_report.py --input_file=${FILE_NAME} --folder=${RES_FOLDER} --parameter_analysis=1 --complex_name=${FILE_NAME}"
echo ${CMD}
...
CMD="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -nodes 128 -q 112500 -strat perm -kmin 20 -kmax 80 -linkspeed 400000 -mtu 2048 -seed 45 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 0 -goal ${Incast_Message_Size} -algorithm delayD -fast_drop 0 -ignore_ecn_data 1 -ignore_ecn_ack 1 -number_entropies -1 -fast_drop 1 > ${RES_FOLDER}/${FILE_NAME}"
echo ${CMD}
eval ${CMD}
CMD="python3 generate_report.py --input_file=${FILE_NAME} --folder=${RES_FOLDER} --parameter_analysis=1 --complex_name=${FILE_NAME}"
echo ${CMD}

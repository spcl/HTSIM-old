...
class Configuration:
    def __init__(self, k, fd, fi, super_fi, x, z, y, bonus_drop, algo_name, runtime_vs_ndp=0, maxvsminfair=0):
        self.k = k
        self.fd = fd
        self.fi = fi
...
with open("Results_RunTime_Good.txt", 'w') as f:
    ...
    for good_ele in good_config:
        f.write('Config --> Algo: {} - KMin: {} - FastDrop: {} - FastInc: {} - SuperFastInc: {} - X: {} - Y: {} - W: {} - Z: {} - BonusDrop: {} -. Res {} {}\n'.format(good_ele.algo_name, good_ele.k, good_ele.fd, good_ele.fi, good_ele.super_fi, good_ele.x, good_ele.y, 4, good_ele.z, good_ele.bonus_drop, good_ele.my_results, good_ele.my_results_raw)) 

with open("Results_RunTime_Bad.txt", 'w') as f:
    f.write("Experiments order: {}\n\n".format(list_experiments))
    for bad_ele in filtered_out_value:
        f.write('Config --> Algo: {} - KMin: {} - FastDrop: {} - FastInc: {} - SuperFastInc: {} - X: {} - Y: {} - W: {} - Z: {} - BonusDrop: {} -. Res {} {}\n'.format(bad_ele.algo_name, bad_ele.k, bad_ele.fd, bad_ele.fi, bad_ele.super_fi, bad_ele.x, bad_ele.y, 4, bad_ele.z, bad_ele.bonus_drop, bad_ele.my_results, bad_ele.my_results_raw)) 
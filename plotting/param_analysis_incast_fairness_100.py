...
class Configuration:
    def __init__(self, k, fd, fi, super_fi, x, z, y, bonus_drop, buffer_drop, algo_name, runtime_vs_ndp=0, maxvsminfair=[]):
        self.k = k
        self.fd = fd
        self.fi = fi
    ...
    def __eq__(self, other):
        return ((self.k == other.k) and (self.fd == other.fd) and (self.fi == other.fi) and (self.super_fi == other.super_fi) and (self.x == other.x) and  (self.z == other.z) and  (self.y == other.y)  and  (self.bonus_drop == other.bonus_drop)  and  (self.buffer_drop == other.buffer_drop) and  (self.algo_name == other.algo_name)) 
                ...
                for use_fast_inc in use_fast_incs:
                    for use_super_fast_increase in use_super_fast_increases:
                        for x in x_list:
                            for z in z_list:
                                for y in y_list:
                                    for buffer_drop in buffer_drops
                                        for bonus_drop in bonus_drops:
                                            for bad_config in filtered_out_value:
                                                # Check if the config we are using is bad
                                                if (bad_config.k == kmin and bad_config.fd == use_fast_drop and bad_config.fi == use_fast_inc and bad_config.super_fi == use_super_fast_increase and bad_config.x == x and bad_config.z == z and bad_config.y == y  and bad_config.bonus_drop == bonus_drop  and bad_config.buffer_drop == buffer_drop and bad_config.algo_name == algorithm_name):
                                                    should_skip = True
                                                    break
                                            ...
                                            # Run My Exp
                                            config_run += 1
                                            FILE_NAME="LinkSpeed{}_Type{}_Version{}_KMin{}_UseFastDrop{}_UseFastIncrease{}_DoSuperFastInc{}_X{}_Z{}_Y{}_BonusDrop{}_BufferDrop{}".format(link_speed, incast_degree_and_size.replace(".", "_" ), algorithm_name, kmin, use_fast_drop, use_fast_inc, use_super_fast_increase, x, y, z, bonus_drop, buffer_drop)
                                            FILE_NAME = FILE_NAME.replace(".", "")
                                            nodes_num = extract_first_number(incast_degree_and_size)
                                            cmd="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -algorithm {} -nodes {} -q {} -strat perm -kmin {} -kmax 80 -target_rtt_percentage_over_base {}  -use_fast_increase {} -use_super_fast_increase {} -fast_drop {} -linkspeed {} -mtu 2048 -seed 99 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 0 -goal {} -ignore_ecn_data 1 -ignore_ecn_ack 1 -number_entropies -1 -x_gain {} -y_gain {} -w_gain {} -z_gain {} -bonus_drop {} -drop_value_buffer {} > {}/{}".format(algorithm_name, nodes_num, cwnd_and_buffer, kmin, rtt_target_fixed, use_fast_inc, use_super_fast_increase, use_fast_drop,  link_speed, incast_degree_and_size, x, y, w, z, bonus_drop, buffer_drop, RES_FOLDER, FILE_NAME)
                                            print(cmd)
                                            os.system(cmd)
                                            ...
                                            if (Configuration(kmin, use_fast_drop, use_fast_inc, use_super_fast_increase, x, z, y, bonus_drop, buffer_drop, algorithm_name) not in good_config):
                                                good_config.append(Configuration(kmin, use_fast_drop, use_fast_inc, use_super_fast_increase, x, z, y, bonus_drop, buffer_drop, algorithm_name))
                                            ...
                                            if (min_vs_max_fct > 30):
                                                if (Configuration(kmin, use_fast_drop, use_fast_inc, use_super_fast_increase, x, z, y, bonus_drop, buffer_drop, algorithm_name) not in filtered_out_value):
                                                    killed_config += 1
                                                    ...
                                                    for idx, config in enumerate(filtered_out_value):
                                                        if (config.k == kmin and config.fd == use_fast_drop and config.fi == use_fast_inc and config.super_fi == use_super_fast_increase and config.x == x and config.z == z and config.y == y  and config.bonus_drop == bonus_drop and config.buffer_drop = buffer_drop  and config.algo_name == algorithm_name):
...
with open("Results_Fairness_Good_100.txt", 'w') as f:
    f.write("Experiments order: {}\n\n".format(list_experiments))
    for good_ele in good_config:
        f.write('Config --> Algo: {} - KMin: {} - FastDrop: {} - FastInc: {} - SuperFastInc: {} - X: {} - Y: {} - W: {} - Z: {} - BonusDrop: {} - BufferDrop: {} -- Res {} {} {}\n'.format(good_ele.algo_name, good_ele.k, good_ele.fd, good_ele.fi, good_ele.super_fi, good_ele.x, good_ele.y, 4, good_ele.z, good_ele.bonus_drop, good_ele.buffer_drop, good_ele.my_results, good_ele.my_results_raw, good_ele.maxvsminfair)) 

with open("Results_Fairness_Bad_100.txt", 'w') as f:
    f.write("Experiments order: {}\n\n".format(list_experiments))
    for bad_ele in filtered_out_value:
        f.write('Config --> Algo: {} - KMin: {} - FastDrop: {} - FastInc: {} - SuperFastInc: {} - X: {} - Y: {} - W: {} - Z: {} - BonusDrop: {} - BufferDrop: {} -- Res {} {} {}\n'.format(bad_ele.algo_name, bad_ele.k, bad_ele.fd, bad_ele.fi, bad_ele.super_fi, bad_ele.x, bad_ele.y, 4, bad_ele.z, bad_ele.bonus_drop, bad_ele.buffer_drop, bad_ele.my_results, bad_ele.my_results_raw, bad_ele.maxvsminfair)) 
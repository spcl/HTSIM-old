...
# List Experiments
list_experiments = os.listdir('../sim/lgs/input/incast_normal')
...
def getRunStringUnbalanced(algo, exp, os_ratio):
    ...
    if (algo == "SMaRTT"):
            string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k {} -algorithm delayB -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal incast_unbalanced/{} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(os_ratio, num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "SMaRTT_ECN"):
        string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k {} -algorithm ecn -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal incast_unbalanced/{} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(os_ratio, num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "SMaRTT_RTT"):
        string_to_run="../sim/datacenter/htsim_uec_entry_modern -o uec_entry -k {} -algorithm rtt -nodes {} -q 445200 -strat ecmp_host_random2_ecn -number_entropies 256 -kmin 20 -kmax 80 -target_rtt_percentage_over_base 50  -use_fast_increase 1 -use_super_fast_increase 1 -fast_drop 1 -linkspeed {} -mtu {} -seed 919 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 1 -goal incast_unbalanced/{} -ignore_ecn_data 1 -ignore_ecn_ack 1 -x_gain {}  -y_gain {} -w_gain {} -z_gain {} -bonus_drop 0.8 -collect_data {} -drop_value_buffer 1.0 > {}/{}/P".format(os_ratio, num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, X_GAIN, Y_GAIN, W_GAIN, Z_GAIN, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "Swift"):
        string_to_run="../sim/datacenter/htsim_swift_entry -k {} -ratio_os_stage_1 1 -o uec_entry -nodes {} -q 448500 -strat ecmp_host_random2_ecn -kmin 20 -target_rtt_percentage_over_base 50 -kmax 80 -linkspeed {} -mtu {} -seed 95 -queue_type composite -hop_latency 700 -switch_latency 0 -reuse_entropy 0 -goal incast_unbalanced/{} -number_entropies 256  -collect_data {}  > {}/{}/P".format(os_ratio, num_nodes, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)
    elif (algo == "EQDS"):
        string_to_run="../sim/datacenter/htsim_ndp_entry_modern -o uec_entry -nodes {} -k {} -kmin 20 -kmax 80 -ratio_os_stage_1 1 -cwnd 448500 -q 448500 -strat ecmp_host_random2_ecn -linkspeed {} -mtu {} -seed 95 -hop_latency 700 -switch_latency 0 -goal incast_unbalanced/{} -collect_data {} -number_entropies 256 > {}/{}/P".format(num_nodes, os_ratio, LINK_SPEED_VALUE, MTU_SIZE, exp, COLLECT_DATA, FOLDER, SUB_FOLDER)
...
for os_rat in list_os:
    for exp_size in overall_list:
        ...
        if (os_rat == 1):
            FOLDER="400_INCAST_NO_OS_{}".format(third_number)
        else:
            FOLDER="400_INCAST_OS4:1_{}".format(third_number)
...
# List Experiments
list_experiments = os.listdir('../sim/lgs/input/incast_unbalanced')
print(list_experiments)
list_algorithm = ["SMaRTT", "SMaRTT_ECN", "SMaRTT_RTT", "Swift", "EQDS"]
list_os = [1, 4]
sizes_normal = [4096, 1048576, 16777216, 134217728]
...
for os_rat in list_os:
    for exp_size in overall_list:
        ...
        if (os_rat == 1):
            FOLDER="400_INCASTU_NO_OS_{}".format(third_number)
        else:
            FOLDER="400_INCASTU_OS4:1_{}".format(third_number)
        CMD="rm -r {}".format(FOLDER)
        os.system(CMD)
        CMD="mkdir {}".format(FOLDER)
        os.system(CMD)
